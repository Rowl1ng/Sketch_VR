...
public static class FileDialogForWindows
{
    ...
    [DllImport("Comdlg32.dll", SetLastError = true, ThrowOnUnmappableChar = true, CharSet = CharSet.Auto)]
    static extern bool GetOpenDirName([In, Out]DialogConfig dialog);  //这个方法名称必须为GetOpenDirName
    [DllImport("Comdlg32.dll", SetLastError = true, ThrowOnUnmappableChar = true, CharSet = CharSet.Auto)]
    ...
    public static string DirDialog(string title, params string[] extensions)
    {
        DialogConfig ofn = new DialogConfig();
        ofn.structSize = Marshal.SizeOf(ofn);
        ofn.maxFileTitle = ofn.fileTitle.Length;
